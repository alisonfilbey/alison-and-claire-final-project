...
alt.Chart(severe_crashes_by_speed).mark_bar(color="olivedrab", opacity=0.85
...
ped_crashes = pd.merge(ped_crashes, ped_actions, on="CRASH_RECORD_ID", how="inner")
ped_action = ped_crashes[ped_crashes['n_peds_total']==1]

#subset to tope ten actions 
...
alt.Chart(ped_action, title= "Number of Crashes Involving Pedestrians by the Action of the Pedestrian").mark_bar(color="seagreen", opacity=0.85).encode(
    x=alt.Y("sum(n_peds_severe):Q", axis=alt.Axis(title= 'Number')),
    y=alt.X('PEDPEDAL_ACTION:O', sort='x', axis=alt.Axis(title= 'Action of Pedestrian', labelFontSize=6))
)
```
...
ped_crashes["geometry"] = ped_crashes["geometry"].apply(wkt.loads)
